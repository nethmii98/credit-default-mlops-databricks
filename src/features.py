...
def add_ratio_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["utilization_latest"] = _safe_divide(
        out["bill_amount_sep"], out["credit_limit"]
    )
    ...
    out["pay_to_bill_ratio_sep"] = _safe_divide(
        out["payment_amount_sep"], out["bill_amount_sep"]
    )

    ratio_cols = []
    for month in months:
        col_name = f"pay_to_bill_ratio_{month}"
        out[col_name] = _safe_divide(
            out[f"payment_amount_{month}"], out[f"bill_amount_{month}"]
        )
...
def add_customer_profile_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["limit_bal_x_late_payment_count"] = (
        out["credit_limit"] * out["late_payment_count_6m"]
    )
