...
pred_prob_col = "default_probability" if "default_probability" in inference_pdf.columns else None
if pred_prob_col is None:
    raise ValueError("Expected column 'default_probability' not found in inference data.")
...
prediction_summary = {
    ...
    "high_risk_proportion": float((inference_pdf[pred_prob_col] >= HIGH_RISK_THRESHOLD).mean()),
...
if missing_feature_cols:
    print(f"Warning: missing feature columns in inference source: {missing_feature_cols}")
...
spark.createDataFrame(prediction_summary_pdf).write.format("delta").mode("append").saveAsTable(MONITORING_SUMMARY_TABLE)

if not counts_by_model_pdf.empty:
    spark.createDataFrame(counts_by_model_pdf).write.format("delta").mode("overwrite").saveAsTable("credit_default_counts_by_model")

if not feature_drift_pdf.empty:
    spark.createDataFrame(feature_drift_pdf).write.format("delta").mode("append").saveAsTable(FEATURE_DRIFT_TABLE)

if not performance_pdf.empty:
    spark.createDataFrame(performance_pdf).write.format("delta").mode("append").saveAsTable(PERFORMANCE_TABLE)
